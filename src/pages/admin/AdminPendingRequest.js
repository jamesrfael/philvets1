...
const AdminPendingRequest = () => {
  ...
  const [requests, setRequests] = useState([]); // Assuming initial requests data
  ...
  return (
    <MainLayout>
      <Controls>
        ...
        />
        <Button onClick={openAddRequestModal}>Add Request</Button>
      </Controls>
      <AnalyticsContainer>
        <div onClick={() => navigate("/admin/orders/pending-request")}>
          <CardTotalPendingRequest />
        </div>
      </AnalyticsContainer>
      <Table headers={headers} rows={rows} />
      {selectedRequest && (
        <OrderDetailsModal request={selectedRequest} onClose={closeDetailsModal} />
      )}
      {isAddingRequest && (
        <AddRequestModal
