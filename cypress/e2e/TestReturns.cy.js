...
roles.forEach(role => {
  describe(`${role.charAt(0).toUpperCase() + role.slice(1)} Returns Tests`, () => {
    beforeEach(() => {
    ...
    describe('Search Bar Tests', () => {
      it('Should display the search bar', () => {
        cy.get('input[placeholder="Search / Filter returns..."]').should('be.visible');
      });
    ...
    it('Should display total returns', () => {
      cy.get('div').contains('Total Returns').should('exist');
    });
  });
});
